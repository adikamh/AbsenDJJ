...
- **Pemisahan Berkas Inline CSS & JS dari View Blade (Refactoring)**:
  - **Prinsip Modularisasi & Keamanan**: Memindahkan seluruh kode `<style>` dan `<script>` yang sebelumnya menyatu di dalam berkas Blade ke berkas asset terpisah di `resources/css` dan `resources/js`, dengan nama berkas yang disesuaikan persis dengan nama file Bladenya.
  - **Penerapan pada Login Halaman (login.blade.php)** ([login.blade.php](file:///c:/laragon/www/AbsenDJJ/resources/views/auth/login.blade.php), [auth-login.js](file:///c:/laragon/www/AbsenDJJ/resources/js/auth-login.js)): Mengekstrak modal notifikasi SweetAlert2 ke berkas Javascript dengan melewatkan data error melalui elemen penanda `#login-error-flag` dengan atribut data.
  - **Penerapan pada Dashboard Pembimbing (dashboard.blade.php)** ([dashboard.blade.php](file:///c:/laragon/www/AbsenDJJ/resources/views/dashboard/admin/dashboard.blade.php), [dashboard.js](file:///c:/laragon/www/AbsenDJJ/resources/js/admin/dashboard.js)): Memisahkan rendering grafik Chart.js dari Blade ke Javascript dengan melewatkan parameter dataset chart menggunakan atribut `data-hadir`/`data-terlambat` dan `data-compliance` JSON terenkode.
  - **Penerapan pada Detail Intern Pembimbing (show.blade.php)** ([show.blade.php](file:///c:/laragon/www/AbsenDJJ/resources/views/dashboard/admin/interns/show.blade.php), [show.css](file:///c:/laragon/www/AbsenDJJ/resources/css/show.css), [show.js](file:///c:/laragon/www/AbsenDJJ/resources/js/show.js)): Memindahkan efek hover foto selfie ke `show.css` dan kontrol modal pop-up detail selfie kehadiran ke `show.js`.
  - **Penerapan pada Izin Peserta (leave.blade.php)** ([leave.blade.php](file:///c:/laragon/www/AbsenDJJ/resources/views/dashboard/peserta/leave.blade.php), [leave.css](file:///c:/laragon/www/AbsenDJJ/resources/css/leave.css), [leave.js](file:///c:/laragon/www/AbsenDJJ/resources/js/leave.js)): Memisahkan baris penyaringan/filter responsif ke `leave.css` dan fungsi toggle modal ajukan permohonan ke `leave.js`.
  - **Penerapan pada Logbook Peserta (logbook.blade.php)** ([logbook.blade.php](file:///c:/laragon/www/AbsenDJJ/resources/views/dashboard/peserta/logbook.blade.php), [logbook.css](file:///c:/laragon/www/AbsenDJJ/resources/css/logbook.css), [logbook.js](file:///c:/laragon/www/AbsenDJJ/resources/js/logbook.js)): Memisahkan badge status rancangan (draft) ke `logbook.css` serta penanganan modal edit & dialog konfirmasi hapus logbook berbasis SweetAlert2 ke `logbook.js`.
  - **Penerapan pada Parameter Global (settings.blade.php)** ([settings.blade.php](file:///c:/laragon/www/AbsenDJJ/resources/views/dashboard/super_admin/settings.blade.php), [settings.js](file:///c:/laragon/www/AbsenDJJ/resources/js/super_admin/settings.js)): Mengonversi objek konfigurasi global `window.settingsConfig` dari inline script menjadi elemen `#settings-config` dengan atribut data untuk dibaca secara dinamis dan aman oleh `settings.js`.
  - **Vite & Laravel Integration** ([vite.config.js](file:///c:/laragon/www/AbsenDJJ/vite.config.js)): Mendaftarkan seluruh berkas asset baru tersebut ke konfigurasi input Vite sehingga terkompilasi optimal untuk produksi.
- **Pemisahan & Modularisasi Controller Admin (Refactoring)**:
  - **Masalah Pengelompokan Logika**: Sebelumnya, listing halaman logbook dan permohonan izin berada di `InternController.php`, sedangkan aksi penyetujuan/penolakan (approve/reject) justru tercampur di dalam `DashboardController.php`.
  - **Pemisahan Logbook (LogbookController.php)** ([LogbookController.php](file:///c:/laragon/www/AbsenDJJ/app/Http/Controllers/Admin/LogbookController.php)): Membuat controller tersendiri untuk menangani seluruh logika logbook admin, memuat halaman daftar logbook bimbingan (`index`) sekaligus method persetujuan (`approve`) dan penolakan (`reject`).
  - **Pemisahan Perizinan (LeaveController.php)** ([LeaveController.php](file:///c:/laragon/www/AbsenDJJ/app/Http/Controllers/Admin/LeaveController.php)): Membuat controller tersendiri untuk seluruh logika permohonan izin/sakit admin, memuat halaman daftar izin (`index`) serta method persetujuan (`approve`) dan penolakan (`reject`).
  - **Penyederhanaan Dashboard & Intern Controller**: Memotong kode-kode method yang dipindahkan sehingga `DashboardController.php` hanya bertanggung jawab atas tampilan ringkasan dashboard, dan `InternController.php` hanya berfokus pada data profil serta detail aktivitas harian anak bimbingan.
  - **Pembaruan Pemetaan Rute (routes/web.php)** ([web.php](file:///c:/laragon/www/AbsenDJJ/routes/web.php)): Menyesuaikan penunjukan kelas controller pada seluruh rute admin agar mengarah ke `LogbookController` dan `LeaveController` yang baru secara teratur.
